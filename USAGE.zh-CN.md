...
PDF 导入时存在以下硬限制（超限数据会被静默丢弃）：

| 限制 | 数值 | 效果 |
|------|------|------|
| 文本层索引最大页数 | 400 | 超过 400 页的部分不索引 |
| 单次 prompt 文本上限 | 80 KB | 截断，末尾标记 `[...truncated...]` |
| 分块索引触发条件 | >60 页或 >80 KB 文本 | 改为逐块 LLM 索引 |
| 最大提取图片数 | 30 / 文档 | 超出部分丢弃 |
| 每页最大图片数 | 5 | 同页更多图片丢弃 |
| 图片最小尺寸 | 100 px / 512 B | 过小图片自动过滤 |
| 扫描件 OCR 触发 | 平均 <50 字符/页 | 如有视觉模型配置则走 OCR |

**读取时软限制**（agent 可通过翻页跨越）：

| 限制 | 数值 | 效果 |
|------|------|------|
| 默认读取页数 | 20 | agent 未指定范围时的默认值 |
| 单次最大读取页数 | 50 | 每次 `read_files` 调用的上限 |
| 无范围全文搜索 | 200 页 | 不带页范围的模式搜索上限 |

覆盖信息存储在 `description.coverage` 中，`--json` 输出包含 `indexed_partial`、`max_index_pages`、`partial_reasons` 字段。
